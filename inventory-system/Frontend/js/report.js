...
const totalQuantity = App.sumQuantities(filtered)
const stockInQuantity = App.sumQuantities(filtered.filter((item)=>item.type === "IN"))
const stockOutQuantity = App.sumQuantities(filtered.filter((item)=>item.type === "OUT"))
const topProduct = App.findTopProduct(filtered)
...
if(latestTime){
latestTime.innerText = latest ? App.formatThaiDate(latest.created_at) : "-"
}
}
